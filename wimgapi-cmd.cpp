...
// 定义必要的WIM消息类型和常量
constexpr auto WIM_MSG_PROGRESS = 0x00000002;
...
typedef DWORD(WINAPI* WIMRegisterMessageCallbackPtr)(
    ...
    _In_     WIM_MSG_CALLBACK fpMessageProc,
...
BOOL InstallImage(LPCWSTR wimPath, DWORD imageIndex, LPCWSTR installPath) {
    ...
    pfnWIMRegisterMessageCallback(NULL, ProgressCallback, NULL);

    if (pfnWIMApplyImage(hImage, installPath, 0)) {
    ...
    pfnWIMUnregisterMessageCallback(NULL, ProgressCallback);
